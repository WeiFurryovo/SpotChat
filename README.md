...
- Wear OS 优先的 Wear Compose 界面，按圆表和方表分别调整安全区、内容宽度和可见消息数量，中文 UI。
- 左滑或点击头像进入个人资料页，个人资料页使用 Wear `ScreenScaffold`、`ScalingLazyColumn` 和滚动指示器上下浏览头像；返回按钮、系统返回或 Wear 右滑返回聊天页，页面切换带滑入动画。
